...
class CatVTONPipeline:
    def __init__(
        ...
        if not skip_safety_check:
            ...
            self.safety_checker = StableDiffusionSafetyChecker.from_pretrained(base_ckpt, subfolder="safety_checker").to(device, dtype=torch.float32)
    ...
    def run_safety_checker(self, image):
        if self.safety_checker is None:
            ...
            image, has_nsfw_concept = self.safety_checker(
                images=image, clip_input=safety_checker_input.pixel_values.to(torch.float32)
            )
